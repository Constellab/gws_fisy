...
@app_decorator("FisyAppStAppConfig", app_type=AppType.STREAMLIT,
               human_name="Generate FisyAppSt app")
class FisyAppStAppConfig(AppConfig):

    # retrieve the path of the app folder, relative to this file
    # the app code folder starts with a underscore to avoid being loaded when the brick is loaded
    def get_app_folder_path(self):
        return self.get_app_folder_from_relative_path(__file__, "_fisy_app_st")


@task_decorator("GenerateFisyAppSt", human_name="Generate FisyAppSt app",
                style=StreamlitResource.copy_style())
class GenerateFisyAppSt(Task):
    """
    Task that generates the FisyAppSt app.
    """
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        streamlit_app.set_app_config(FisyAppStAppConfig())
        streamlit_app.name = "FisyAppSt"
