...
class State(ReflexMainState):
    ...
    def currency_symbol(self) -> str:
        return {"EUR": "€", "USD": "$", "GBP": "£", "CHF": "CHF", "CAD": "$", "AUD": "$", "JPY": "¥"}.get(
    ...
    def set_int(self, field: str, v: str):
        ...
        if "month" in field or "months" in field:
            val = max(1, val)
        else:
            val = max(0, val)
    ...
    def synthese_table_annual_cols(self) -> list[str]:
        return self._cols_vals(self._monthly_to_annual(self._dfs()["synthese"]), "Année", ["Année"])[0]

    @rx.var
    def synthese_table_annual_values(self) -> list[list[str]]:
        return self._cols_vals(self._monthly_to_annual(self._dfs()["synthese"]), "Année", ["Année"])[1]

    @rx.var
    def synthese_table_monthly_cols(self) -> list[str]:
        return self._cols_vals(self._dfs()["synthese"], "Mois")[0]

    @rx.var
    def synthese_table_monthly_values(self) -> list[list[str]]:
        return self._cols_vals(self._dfs()["synthese"], "Mois")[1]

    # ========== P&L ==========
    @rx.var
    def pnl_table_annual_cols(self) -> list[str]:
        return self._cols_vals(self._monthly_to_annual(self._dfs()["pnl"]), "Année", ["Année"])[0]

    @rx.var
    def pnl_table_annual_values(self) -> list[list[str]]:
        return self._cols_vals(self._monthly_to_annual(self._dfs()["pnl"]), "Année", ["Année"])[1]

    @rx.var
    def pnl_table_monthly_cols(self) -> list[str]:
        return self._cols_vals(self._dfs()["pnl"], "Mois")[0]

    @rx.var
    def pnl_table_monthly_values(self) -> list[list[str]]:
        return self._cols_vals(self._dfs()["pnl"], "Mois")[1]

    # ========== CASHFLOW ==========
    @rx.var
    def cashflow_table_annual_cols(self) -> list[str]:
        return self._cols_vals(self._monthly_to_annual(self._dfs()["cashflow"]), "Année", ["Année"])[0]

    @rx.var
    def cashflow_table_annual_values(self) -> list[list[str]]:
        return self._cols_vals(self._monthly_to_annual(self._dfs()["cashflow"]), "Année", ["Année"])[1]

    @rx.var
    def cashflow_table_monthly_cols(self) -> list[str]:
        return self._cols_vals(self._dfs()["cashflow"], "Mois")[0]

    @rx.var
    def cashflow_table_monthly_values(self) -> list[list[str]]:
        return self._cols_vals(self._dfs()["cashflow"], "Mois")[1]

    # ========== PLAN DE FINANCEMENT (annuel) ==========
    @rx.var
    def plan_table_annual_cols(self) -> list[str]:
        return self._cols_vals(self._dfs()["plan_financement"], "Année", ["Année"])[0]

    @rx.var
    def plan_table_annual_values(self) -> list[list[str]]:
        return self._cols_vals(self._dfs()["plan_financement"], "Année", ["Année"])[1]

    # ========== BILANS (mensuel) ==========
    @rx.var
    def bilan_actif_table_cols(self) -> list[str]:
        return self._cols_vals(self._dfs()["bilans"]["actif"], "Mois")[0]

    @rx.var
    def bilan_actif_table_values(self) -> list[list[str]]:
        return self._cols_vals(self._dfs()["bilans"]["actif"], "Mois")[1]

    @rx.var
    def bilan_passif_table_cols(self) -> list[str]:
        return self._cols_vals(self._dfs()["bilans"]["passif"], "Mois")[0]

    @rx.var
    def bilan_passif_table_values(self) -> list[list[str]]:
        return self._cols_vals(self._dfs()["bilans"]["passif"], "Mois")[1]
