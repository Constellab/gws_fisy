...
def one_time():
    ...
    def row(r, irow):
        return rx.table.row(
            make_cell(rx.select(items=State.activity_options, value=r.activity, on_change=lambda v, irow=irow: State.update_item("one_time_ranges", irow, "activity", v, "str")), WIDTHS[0]),
            make_cell(rx.input(type="number", min=1, value=r.start_month, on_change=lambda v, irow=irow: State.update_item("one_time_ranges", irow, "start_month", v, "int")), WIDTHS[1]),
            make_cell(rx.input(type="number", min=1, value=r.end_month, on_change=lambda v, irow=irow: State.update_item("one_time_ranges", irow, "end_month", v, "int")), WIDTHS[2]),
            make_cell(rx.input(type="number", min=0, value=r.q0, on_change=lambda v, irow=irow: State.update_item("one_time_ranges", irow, "q0", v, "float")), WIDTHS[3]),
            make_cell(rx.input(type="number", step=0.01, min=0, value=r.monthly_growth, on_change=lambda v, irow=irow: State.update_item("one_time_ranges", irow, "monthly_growth", v, "float")), WIDTHS[4]),
            actions_cell("one_time_ranges", irow),
        )
    return layout(rx.vstack(
        rx.heading(i["one_time.title"], size="5"),
